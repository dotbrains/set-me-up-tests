...
if [[ "${SMU_PASS_HOST_ENV:-false}" == "true" ]]; then
    for var in SMU_BLUEPRINT SMU_BLUEPRINT_BRANCH SMU_BLUEPRINT_SHA SMU_MODULES SMU_RUN_IDEMPOTENCY SMU_EXPECTED_SYMLINKS SMU_HOME_DIR SMU_INSTALLER_REF SMU_INSTALLER_URL; do
        if [[ -n "${!var:-}" ]]; then
